    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-4 lg:gap-y-0">
        ...
        @foreach ($employee as $item)
            <div class="bg-white rounded-md text-center ease-in duration-200 drop-shadow-md">
                <img class="w-full object-cover bg-center rounded-tl-md rounded-tr-md" src="/img/{{ $item->image }}" alt="">
                <section class="flex flex-col p-4">
                    <h5 class="text-base font-bold mb-1">{{ $item->nama }}</h5>
                    <span class="text-sm font-normal text-gray-500">{{ $item->jabatan }}</span>
                    <div class="flex flex-row gap-x-4 mt-4">
                        <div class="w-full py-2 bg-[#7000FD] text-white rounded-md">
                            <a href={{ route('employee.show',$item->id) }}>Lihat Detail</a>
                        </div>
                        <form action="{{ route('employee.destroy',$item->id) }}" method="POST" class="flex flex-row gap-x-2">
                            <button>
                                <a href={{ route('employee.edit',$item->id) }}><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></a>
                            </button>
                            @csrf
                            @method('DELETE')
                            <button type="submit"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                        </form>
                    </div>
                </section>
            </div>
